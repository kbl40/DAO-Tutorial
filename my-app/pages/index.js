import { Contract, provider, providers } from "ethers";
import { formatEther } from "ethers/lib/utils";
import Head from 'next/head'
import { useEffect, useRef, useState } from "react";
import Web3Modal from "web3modal";
import { CRYPTODEVS_DAO_CONTRACT_ADDRESS, CRYPTODEVS_NFT_CONTRACT_ADDRESS} from "../constants";
...
export default function Home() {
  ...
  // Arral of all proposals created in the DAO
  ...
  // True if waiting on a transaction to be mined, false otherwise
  ...
  // Reads the number or proposals in the DAO contract and sets the `numProposals` state variable
  ...
  // Helper function to fetch and parse one proposal from the DAO contract given the Proposal ID
  ...
  const getDaoContractInstance = (providerOrSigner) => {
    return new Contract(CRYPTODEVS_DAO_CONTRACT_ADDRESS, daoABI.abi, providerOrSigner);
  };

  // Helper function to return a CryptoDevs nFT Contract instance given a Provider/Signer
  const getCryptodevsNFTContractInstance = (providerOrSigner) => {
    return new Contract(CRYPTODEVS_NFT_CONTRACT_ADDRESS, nftABI.abi, providerOrSigner);
  };

  // Piece of code that runs everytime the value of `walletConnected` changes so when a wallet connects
  // or disconnects it prompts user to connect wallet if not connected and then calls helper 
  // functions to fetch the DAO Treasury Balance, User NFTn Balance, and # of proposals in DAO
  useEffect(() => {
    if (!walletConnected) {
  ...
  // Piece of code that runs everytime the value of `selectedTabl` changes
